...
## 2026-06-26 追記 — めくり後のページ編集モーダルのズレを修正 ✅
- **症状（ユーザー報告）**: 本のページをめくった後、本の**右側**をダブルクリックすると、
  「めくる前のページ(page 3-4)」の編集モーダルが開いてしまっていた。
- **原因**: canvas ダブルクリック→`book3d-edit-request` を受ける `TemplateBuilder.tsx` の `onReq` が、
  左右を**固定で** `左=page 1-2 / 右=page 3-4` にマッピングしていて、めくり状態を一切見ていなかった。
- **修正**: `src/components/TemplateBuilder.tsx` の `onReq`。現在時刻 `playheadRef.current`(60fps最新) で
  **完了しためくり枚数 N**（`t >= atSec + durationSec` の bookFlip 件数）を数え、
  `左 = spread[N] / 右 = spread[N+1]`、slot 名は `page {2i+1}-{2i+2}` で生成して開くようにした。
  - N=0: 左 page 1-2 / 右 page 3-4（従来どおり）。N=1: 左 page 3-4 / 右 **page 5-6**（めくると右に出るページ）。
  - パネルの「✎ドラッグ編集」ボタン（`detail.slot` 明示）は従来どおり最優先＝不変。
- **検証状況**: N=1（今回の単一めくり）は引き継ぎ記述・テンプレ実データと一致＝確実。
  **N≥2（複数めくり）の slot 対応は未検証**（テンプレに page 7-8 以降の実データが無い）。
  「めくり1回=見開き1つ前進」「左=spread[N]/右=spread[N+1]」は妥当な一般化だが要実機確認。
  しきい値は「めくり**完了後**」基準（最中は左右が曖昧なので停止状態の編集を想定）。
- ビルド済み（tauri build → NSIS /S 再インストールで AppData の exe まで反映）。
